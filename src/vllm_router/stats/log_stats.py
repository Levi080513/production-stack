    ...
    while True:
        ...
        for endpoint in endpoints:
            ...
            if url in engine_stats:
                ...
                gpu_prefix_cache_hit_rate.labels(workspace=endpoint.workspace,endpoint=endpoint.endpoint, server=url).set(
                    es.gpu_prefix_cache_hit_rate
                )
                gpu_prefix_cache_hits_total.labels(workspace=endpoint.workspace,endpoint=endpoint.endpoint, server=url).set(
                    es.gpu_prefix_cache_hits_total
                )
                gpu_prefix_cache_queries_total.labels(workspace=endpoint.workspace,endpoint=endpoint.endpoint, server=url).set(
            ...
            if url in request_stats:
                ...
                current_qps.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(rs.qps)
                avg_decoding_length.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(rs.avg_decoding_length)
                num_prefill_requests.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(rs.in_prefill_requests)
                num_decoding_requests.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(rs.in_decoding_requests)
                num_requests_running.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(
                    rs.in_prefill_requests + rs.in_decoding_requests
                )
                avg_latency.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(rs.avg_latency)
                avg_itl.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(rs.avg_itl)
                num_requests_swapped.labels(workspace=endpoint.workspace, endpoint=endpoint.endpoint, server=url).set(rs.num_swapped_requests)
            else:
